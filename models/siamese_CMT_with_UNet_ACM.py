...
#   CMT Configuration   #
#########################
class siamese_CMT_with_UNet_ACM(t.nn.Module):
    """Define CMT model"""
    ...
    def __init__(self,
        ...
        """
        super(siamese_CMT_with_UNet_ACM, self).__init__()
    ...
    def forward(self, x , x2 , base_unet, fu_unet):

        # 1. Stem
        x = t.cat([x, base_unet] , 1)
        x_stem = self.stem(x)

        # 1. Stem
        x2 = t.cat([x2, fu_unet] , 1)
